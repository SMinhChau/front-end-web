...
const GroupDetail = () => {
  ...
  useEffect(() => {
    if (termState.term.length > 0) {
      lecturerService.getGroupLecturers({ termId: termState.termSelected, typeEvaluation: 'REVIEWER' }).then((resutl) => {
        setGroupLecturer(resutl?.data);
      }).catch((er) => console.log("getGroupLecturers REVIEW", er)
      )
      lecturerService.getGroupLecturers({ termId: termState.termSelected, typeEvaluation: 'SESSION_HOST' }).then((resutl) => {
      ...
      >
        <Form.Item label="" rules={[{ required: true }]} name="groupLecturerId">
          <Select
            ...
            onChange={handleChange}
            optionLabelProp="label"
            options={groupLecturerHost.map((value) => {
              return {
                value: value.id,
                label: value.name,
              };
  ...
  }, [statusHost, initDataHost, groupLecturerHost]);
